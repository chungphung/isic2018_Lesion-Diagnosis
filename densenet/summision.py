    ...
    for step in tqdm(range(iteration), desc="Running..."):
        ...
        for i in range(batch_size):
            if 'image' not in result.keys():
                result['image'] = [basename(test_loader.imgs_path[step*batch_size:(step+1)*batch_size][i])[:-4]]
            else:
                result['image'].append(basename(test_loader.imgs_path[step*batch_size:(step+1)*batch_size][i])[:-4])
            for j in range(7):
